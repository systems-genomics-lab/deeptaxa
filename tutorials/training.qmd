...
**Prerequisites**
- A CUDA-capable GPU (training on CPU is possible but roughly 10x slower)
- Approximately 1 GB of disk space for the training data
- 1 to 2 hours for the full training run described here
...
DeepTaxa trains on the [Greengenes 2](https://greengenes2.ucsd.edu/) reference database (release 2024.09), a curated collection of full-length 16S rRNA gene sequences with standardized seven-rank taxonomy annotations. The training subset contains approximately 277,000 sequences spanning over 16,000 species.
...
The `deeptaxa train` command orchestrates this entire pipeline. Only `--fasta-file`, `--taxonomy-file`, and `--output-dir` are required — all other flags default to the Optuna-optimized values used in the published checkpoint.
...
| `--batch-size` | Number of sequences per gradient update | 32 |
| `--learning-rate` | Learning rate | 3.72e-4 |
| `--seed` | Random seed for reproducibility | 42 |
| `--hidden-size` | Transformer hidden dimension | 1024 |
| `--num-hidden-layers` | Number of Transformer layers | 5 |
| `--num-attention-heads` | Number of attention heads | 8 |
| `--intermediate-size` | Transformer feed-forward dimension | 4096 |
| `--embed-dim` | CNN embedding dimension | 896 |
| `--num-filters` | CNN filters per kernel size | 512 |
| `--kernel-sizes` | CNN kernel widths | 5 7 9 |
| `--num-conv-layers` | Number of CNN layers | 1 |
| `--hidden-dropout-prob` | Dropout probability | 0.17 |
...
The training run demonstrated here follows the same procedure, data, seed, and architecture used to produce the published DeepTaxa checkpoint (10 epochs on the full Greengenes dataset), which achieved 93.0% species-level accuracy and 92.0% weighted F1-score.
