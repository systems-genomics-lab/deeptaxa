...
:::{.callout-note}
Because this tutorial installs from the `main` branch, the exact code can change over time. For a reproducible run, record the commit you installed with `git -C ~/deeptaxa-workspace/deeptaxa rev-parse HEAD`, or install a tagged release once the ensemble tools are part of one.
:::
...
This tutorial uses several standard terms: the ensemble, softmax, soft and hard voting, bias and variance, calibration and ECE, and weighted F1. See the [Glossary](glossary.qmd) for short definitions.
