...
Install DeepTaxa from PyPI, along with the optional libraries for evaluation and plotting. DeepTaxa is distributed as `deeptaxa-rrna`; after installation the command-line tool is `deeptaxa`. Bioconda is an alternative for Conda-based toolchains (`conda install -c bioconda deeptaxa-rrna`).

```{python}
#| eval: true
#| label: lst-install-deeptaxa
#| lst-cap: "Install DeepTaxa from PyPI."
#| output: false
%%bash
mkdir -p ~/deeptaxa-workspace
python -m pip install -q deeptaxa-rrna
python -m pip install -q matplotlib scikit-learn
```
...
%%bash
cd ~/deeptaxa-workspace && du -h data/models/* data/greengenes/*
```
...
This tutorial uses several standard terms: accuracy, the 16S rRNA gene, the taxonomic hierarchy, softmax, and byte-pair encoding. See the [Glossary](glossary.qmd) for short definitions.

## Key references

This tutorial uses the Greengenes 2 reference database [@mcdonald2024greengenes] and the DNABERT-2 tokenizer [@zhou2024dnabert2]. See the [References](references.qmd) page for the complete bibliography.
