...
# Close relatives that 16S cannot separate from the expected species are credited here.
# Salmonella bongori and Staphylococcus argenteus are not in this set: the reference
# resolves the true species (enterica, aureus) on the ASVs that carry enough signal, so
# those calls are scored as misses rather than indistinguishable siblings.
SIBLINGS = {
